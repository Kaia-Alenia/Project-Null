...
// Esto elimina el rastro del jugador y arregla la línea gris al mismo tiempo.
...
int main() {
    ...
    while (1) {
        ...
        if (jugador.x == moneda.x && jugador.y == moneda.y) {
            ...
            // Nota: El jugador está encima, así que al restaurar borramos al jugador momentáneamente
            ...
            // Redibujar ambos (Importante porque restaurarCasilla pudo borrar al jugador)
            dibujarJugador(moneda.x, moneda.y, moneda.color);
            dibujarJugador(jugador.x, jugador.y, jugador.color);
        }
    }
